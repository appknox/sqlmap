...
def _comparison(page, headers, code, getRatioValue, pageLength):
    ...
    if kb.testMode:
        threadData.lastComparisonHeaders = listToStrValue(_ for _ in headers.items() if not _[0].startswith("%s:" % URI_HTTP_HEADER)) if headers else ""
    ...
    if any((conf.string, conf.notString, conf.regexp)):
        rawResponse = "%s%s" % (listToStrValue(_ for _ in headers.items() if not _[0].startswith("%s:" % URI_HTTP_HEADER)) if headers else "", page)
